...
for session_idx in range(len(session_directories)):
    ...
    for step in range(max_iteration):
        ...
        if method == 'reactive':
            ...
            grasp_success_over_interval = np.sum(reward_value_log[grasp_attempt_ind_over_interval] == 1.0)/float(min(interval_size,max(step,1))) # Reward value for successful grasping is anything larger than 0.5 (reinforcement policy)
    ...
    plt.plot(range(0, max_iteration), 100*grasp_success, color=color, linewidth=1.5) # color='blue', linewidth=3)
